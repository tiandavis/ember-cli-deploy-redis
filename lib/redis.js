...
module.exports = CoreObject.extend({
  ...
  upload: function(/*keyPrefix, revisionKey, value*/) {
    ...
    return Promise.resolve()
      ...
      .then(this._setExpire.bind(this, keyPrefix, revisionKey, expire))
      .then(function() {
        return redisKey;
      });
  },
  ...
  _setExpire: function(keyPrefix, revisionKey, expire) {
    var client = this._client;
    var keys = keyPrefix + '*';
    ...
    return Promise.hash({
      revisions: client.send('keys', [keys]),
      current: this.activeRevision(keyPrefix)
    }).then(function(results) {
      ...
      revisions.forEach(function(revision) {
        if (revision !== current) {
          client.send('expire', [keyPrefix + ":" + revision, expire])
        }
      });
    });
  },
